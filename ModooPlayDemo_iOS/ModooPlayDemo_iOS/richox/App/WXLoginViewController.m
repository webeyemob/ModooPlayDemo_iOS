...
@interface WXLoginViewController ()<WXApiDelegate,GIDSignInUIDelegate, GIDSignInDelegate,ASAuthorizationControllerDelegate,ASAuthorizationControllerPresentationContextProviding>
...
- (void)wxLogin:(NSNotification*)noti{
    ...
    if (dictionary != nil && dictionary[@"socialAccount"]) {
        ...
        if (wxDic != nil) {
            ...
            if (userId != nil) {
                [self bindWeChatToFission:RICHOX_BASE_BINDTYPE_WECHAT openId:wxAppID token:_code];
            } else {
                [self registerWechatToFission:RICHOX_BASE_BINDTYPE_WECHAT openId:wxAppID token:_code];
            }
        }
    }
...
//    [self doLoginWithURL:@"userinfo/login_google" Params:@{@"access_token":user.authentication.idToken, @"bean":@([MMCoinManager shareInstance].allCoins * 10)}];
    [self bindWeChatToFission:RICHOX_BASE_BINDTYPE_GOOGLE openId: user.userID token:user.authentication.idToken];
}
...
- (void)fblogin {
    ...
    if ([FBSDKAccessToken currentAccessToken])  {
        [self bindWeChatToFission:RICHOX_BASE_BINDTYPE_FACEBOOK openId:[FBSDKAccessToken currentAccessToken].userID token:[FBSDKAccessToken currentAccessToken].tokenString];
...
//                    [self doLoginWithURL:@"userinfo/login_fb" Params:@{@"user_id":profile.userID, @"access_token":[FBSDKAccessToken currentAccessToken].tokenString, @"bean" : @([MMCoinManager shareInstance].allCoins * 10)}];
                    [self bindWeChatToFission:RICHOX_BASE_BINDTYPE_FACEBOOK openId:profile.userID token:[FBSDKAccessToken currentAccessToken].tokenString];
                }else {
                    [self setupComfirmController:@"网络异常，请稍后重试"];
                }
            }];
...
//            [self bindWeChatToFission:RICHOX_BASE_BINDTYPE_ALIPAY openId:@"alipay" token:authCode];
//        }];
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization API_AVAILABLE(ios(13.0))
{
    if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
        ...
        [self bindWeChatToFission:RICHOX_BASE_BINDTYPE_APPLE openId:userID token:identityToken];
        ...
        //[self bindWeChatToFission:FISSIONSDK_BINDTYPE_GOOGLE openId:user token:identityToken];
    }
}
...
- (BOOL)checkBind:(RICHOX_BASE_BINDTYPE) bindType openId: (NSString *)openId  token: (NSString *) token checkBindResult:(RichOXUserBindResult *)checkBindResult {
    if (checkBindResult.bindType != bindType) {
        return NO;
    }
    
    switch (bindType) {
        case RICHOX_BASE_BINDTYPE_WECHAT: {
            RichOXWechatAccount *account = (RichOXWechatAccount *)(checkBindResult.socialAccountInfo);
            if (account == nil) {
                return NO;
            }
            if ([account.appId isEqualToString:openId] && [account.openId isEqualToString:token]) {
                return YES;
            }
        ...
        case RICHOX_BASE_BINDTYPE_GOOGLE: {
            RichOXGoogleAccount *account = (RichOXGoogleAccount *)(checkBindResult.socialAccountInfo);
            if (account == nil) {
                return NO;
            }
            
            if ([account.googleSub isEqualToString:token]) {
                return YES;
            }
        }
            break;
        ...
        case RICHOX_BASE_BINDTYPE_FACEBOOK: {
            RichOXFacebookAccount *account = (RichOXFacebookAccount *)(checkBindResult.socialAccountInfo);
            if (account == nil) {
                return NO;
            }
            
            if ([account.openId isEqualToString:token]) {
                return YES;
            }
        }
            break;
        ...
        case RICHOX_BASE_BINDTYPE_APPLE: {
            RichOXAppleAccount *account = (RichOXAppleAccount *)(checkBindResult.socialAccountInfo);
            if (account == nil) {
                return NO;
            }
            
            if ([account.appleSub isEqualToString:token]) {
                return YES;
            }
...
//        case RICHOX_BASE_BINDTYPE_ALIPAY: {
//            RichOXAlipayAccount *account = (RichOXAlipayAccount *)(checkBindResult.socialAccountInfo);
//            if (account == nil) {
//                return NO;
//            }
//            
//            if ([account.alipayId isEqualToString:token]) {
//                return YES;
//            }
//            
//        }
...
- (void)bindWeChatToFission:(RICHOX_BASE_BINDTYPE) bindType openId: (NSString *)openId  token: (NSString *) token {
    [RichOXUser checkBind:openId bindType:bindType bindCode:token success:^(RichOXUserBindResult *result) {
        NSLog(@"*******checkBind测试成功: bind result is %@", [result description]);
        if (![self checkBind:bindType openId:openId token:token checkBindResult:result]) {
            [RichOXUser bindUser:openId bindType:bindType bindCode:token success:^(RichOXUserBindResult *result) {
                NSLog(@"*******bindUser测试成功: bindtype is %ld, bind result is %@", bindType, [result description]);
            }  failure:^(NSError *error) {
        ...
        NSLog(@"*******checkBind测试失败: errorCode: %ld, message:%@", error.code, error.localizedDescription);
        [RichOXUser bindUser:openId bindType:bindType bindCode:token success:^(RichOXUserBindResult *result) {
            if (self.bindingCallBack) {
                self.bindingCallBack(YES, @"微信登录成功");
            }
            NSLog(@"*******bindUser测试成功: bindtype is %ld, bind result is %@", bindType, [result description]);
        }  failure:^(NSError *error) {
            if (self.bindingCallBack) {
                self.bindingCallBack(NO, @"微信登录失败");
            }
            NSLog(@"*******bindUser测试失败: errorCode: %ld, message:%@", error.code, error.localizedDescription);
        }];
...
- (void)registerWechatToFission:(RICHOX_BASE_BINDTYPE) bindType openId: (NSString *)openId  token: (NSString *) token {
    [RichOXUser registerByWeChat:openId bindCode:token inviterUid:nil success:^(RichOXUserBindResult * _Nonnull result) {
        NSLog(@"*******registerByWeChat测试成功: bind result is %@", [result description]);
